...
func (suite *OcppV2TestSuite) TestNotifyEventE2EMocked() {
	...
	require.Nil(t, err)
	r, err := suite.chargingStation.NotifyEvent(context.Background(), generatedAt, seqNo, []diagnostics.EventData{eventData}, func(request *diagnostics.NotifyEventRequest) {
		request.Tbc = tbc
	})
	assert.Nil(t, err)
	assert.NotNil(t, r)
